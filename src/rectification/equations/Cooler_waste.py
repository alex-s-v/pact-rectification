...
def Q_feed(deltaT_waste, W_mass, Cw, t1_cooler_waste, ):
    """
...
def m_coolwater_waste(Q_waste_cooler, r_steam):
    """
...
def A_real(Q_feed, Kt_real, deltaT_diff):
    ...
    Романков, формула 4.72, стр.168
    """      
    return Q_wastecooler / (Kt_wastecooler_real * deltaT_diff)


def surface_margin_wastecooler (A_approx_wastecooler, A_real_wastecooler):
    """
    Calculates the surface margin.
    Parameters
    ----------
    A_approximate_wastecooler : float
        The approximate heat ransfer area, [m**2]
    A_real_wastecooler : float
    ...
    &&&&
    """          
    return (A_approx_wastecooler - A_real_wastecooler) * 100 / A_approx